...
dependencies {
  ...
  // implementation(fileTree(System.getenv("BCEL") + "/target") { include("bcel-6.2.0.2.jar") })
...
//
// This is a method of a top-level object rather than of the build script, because the
// configuration cache nulls out a script-defined lambda's owner when it serializes the lambda.
object JavadocFonts {
...
tasks.withType<Javadoc>().configureEach {
  ...
  // Code copied from BCEL that we do not want to change gratuitously.
  exclude("org/plumelib/bcelutil/StackVer.java")
  doLast { destinationDir?.let { JavadocFonts.removeDejaVuFontImport(it) } }
}
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // This is a local variable rather than a variable of the build script, because a top-level
    // variable of a Kotlin build script is a field of the script object, and the configuration
    // cache cannot serialize a task action that refers to that object.
    val markerFile = layout.buildDirectory.file("requireJavadoc/requireJavadoc.txt")
    inputs.files(sourceSets["main"].allJava)
    outputs.file(markerFile)
    mainClass = "org.plumelib.javadoc.RequireJavadoc"
    classpath = requireJavadocConfiguration.get()
...
mavenPublishing {
  ...
  pom {
    name = "BCEL Util"
    description = "Utility functions for BCEL, the Byte Code Engineering Library."
